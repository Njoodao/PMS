  ...
  const kabiDb = {
    ...
    _getTourSteps() {
      ...
      if (!isManager && !isCEO && !isHC) {
        return [
          { tabKey: null, icon: 'ti-hand-stop', title: `Welcome, ${firstName}!`,
            body: `This is a short guided tour to help you get started with KABi. It only takes a minute.` },
          { tabKey: null, icon: 'ti-user-circle', title: 'Your Profile Card',
      ...
      if (isManager) {
        ...
        steps.push({ tabKey: 'eval', icon: 'ti-clipboard-check', title: 'Evaluate Team',
        ...
        steps.push({ tabKey: 'sent', icon: 'ti-send', title: 'Sent Notifications',
        ...
        modal.innerHTML = `
          <div style="position:fixed;inset:0;background:rgba(240,243,250,0.85);backdrop-filter:blur(6px);z-index:2147483647;display:flex;align-items:center;justify-content:center;font-family:'FSAlbert','Segoe UI',Calibri,Arial,sans-serif;">
            <div style="background:#ffffff;padding:32px 36px;border-radius:20px;max-width:440px;width:90%;color:#0f172a;box-shadow:0 30px 70px rgba(19,56,176,0.18),0 4px 12px rgba(0,0,0,0.06);border:1px solid rgba(0,194,224,0.12);">
              <div style="font-size:44px;margin-bottom:10px;line-height:1;color:#00c2e0;"><i class="ti ti-shield-lock"></i></div>
