...
(function () {
  ...
  const kabiDb = {
    ...
    _getTourSteps() {
      ...
      // Lookup employee from KABi's localStorage (source of truth)
      // PE_EMPLOYEES is a `let` inside KABi's IIFE, so not on window — we read
      // KABi's own localStorage key `kabi_pe_employees_v6715` directly.
      let peLookup = null;
      if (effectiveEmail) {
        try {
      ...
      const emp = state.myEmp || (kabiMe ? {
        name: kabiMe.name,
        email: kabiMe.email,
        isHCAdmin: kabiMe.role === 'pm_super_admin' || kabiMe.role === 'hc_admin',
        isCEO: kabiMe.role === 'ceo' || kabiMe.role === 'ceo_exec',
        hasManagerPortal: kabiMe.role === 'manager'
      } : peLookup ? {
        // Best: found in PE_EMPLOYEES — full data available
        name: peLookup.name,
        email: peLookup.email,
        isHCAdmin: peLookup.role === 'pm_super_admin' || peLookup.role === 'hc_admin' || peLookup.isHCAdmin === true,
        isCEO: peLookup.role === 'ceo' || peLookup.isCEO === true,
        hasManagerPortal: peLookup.hasManagerPortal === true
      } : effectiveEmail ? {
        // Last resort: only email — infer role from active DOM page
        name: (state.session?.user?.user_metadata?.name) || effectiveEmail.split('@')[0],
        email: effectiveEmail,
        isHCAdmin: document.getElementById('pg-hc')?.classList.contains('active') === true,
        isCEO: document.getElementById('pg-ceo')?.classList.contains('active') === true,
      ...
      if (!isManager && !isCEO && !isHC) {
        // Employees (Staff level, no sidebar) get ONE brief overview popup — no navigation.
        // NO INVIEWS mention — that feature is for managers who have direct reports.
        return [
          { tabKey: null, icon: 'ti-mood-happy',
            title: `Welcome, ${firstName}!`,
            body: `Welcome to KABi. This is your Employee Workspace where you can view your profile, KPIs (once approved by HC), submit initiatives, complete your well-being check-ins, and follow your notifications. Everything you need is right on this page.` }
        ];
      }
      ...
      // ─── HC Admin tour (uses pg-hc with hc-tabs) ───
      if (isHC) {
        return [
          { tabKey: null, icon: 'ti-mood-happy', title: `Welcome, ${firstName}!`,
            body: `You're now signed in as HC Admin. Let me walk you through the PM Dashboard and each of your main tools.` },
          { tabKey: null, icon: 'ti-chart-bar', title: 'PM Dashboard Overview',
            body: `Your Human Capital command center. The dashboard shows real-time counts — functions, departments, employees, accounts — plus quick access to the four main areas below.` },
          { tabKey: 'hc-pe', icon: 'ti-clipboard-list', title: 'Performance Evaluation',
            body: `Review manager KPI submissions, approve or reject them, track evaluation progress per manager, and release final scores to employees. Also handles edit-request approvals.` },
          { tabKey: 'hc-kpi', icon: 'ti-target', title: 'KPI Framework',
            body: `Review KPI submissions from managers, approve or reject them, manage users & permissions, and edit the curated 173-KPI library across all functions.` },
          { tabKey: 'hc-people', icon: 'ti-users', title: 'People & Access',
            body: `Manage employee records, add or remove users, control access permissions, and reset passwords. Handles the entire employee lifecycle in one place.` },
          { tabKey: 'hc-ai_nudge', icon: 'ti-sparkles', title: 'AI Nudges',
            body: `AI-generated coaching prompts and reminders for managers and employees. Grounded in real evaluation data — never mock or hallucinated.` },
          { tabKey: null, icon: 'ti-trophy', title: 'Release Results',
            body: `Employees see their evaluation scores only after you release them. Two levels: global toggle plus per-employee release. This is intentional — you control the moment of disclosure.` },
          { tabKey: null, icon: 'ti-confetti', title: `You're all set!`,
            body: `You've seen the HC Admin toolkit. Welcome to your command center.` }
        ];
      }
    ...
    _showOnboardingTour() {
      ...
      function render() {
        ...
        // Navigate to the tab (if any) — supports both empSwitchTab (Employee/Manager/CEO)
        // and hcTab (HC Admin). tabKey format: "hc-pe" means call hcTab('pe').
        if (step.tabKey) {
