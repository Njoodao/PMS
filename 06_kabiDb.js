...
(function () {
  ...
  const kabiDb = {
    ...
    _getTourSteps() {
      ...
      const emp = state.myEmp || (kabiMe ? {
        ...
        // Fallback: only Supabase session available, derive from active DOM page
        name: state.session?.user?.user_metadata?.name || supabaseEmail.split('@')[0],
        email: supabaseEmail,
    ...
    _showOnboardingTour() {
      ...
      async function finish() {
        ...
        const email = (window.me?.email || state.myEmp?.email || '').toLowerCase();
        if (email) {
          localStorage.setItem('kabi_tour_completed_' + email, '1');
        }
    ...
    async _restartTour() {
      ...
      const email = (window.me?.email || state.myEmp?.email || '').toLowerCase();
      if (email) {
        localStorage.removeItem('kabi_tour_completed_' + email);
      }
  ...
  const workspaceObserver = new MutationObserver(() => {
    ...
    // Determine current user's email — try multiple sources
    const email = 
         window.me?.email
      || state.myEmp?.email
      ...
      || null;
    if (!email) {
      // Wait a bit longer for KABi to populate window.me
      setTimeout(() => {
        const retryEmail = window.me?.email || state.session?.user?.email;
        if (retryEmail) {
